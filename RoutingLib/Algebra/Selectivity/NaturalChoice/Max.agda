...
module RoutingLib.Algebra.Selectivity.NaturalChoice.Max where
  ...
  module _ {a ℓ} {A : Set a} {_≤_ : Rel A ℓ} (≤-total : Total _≤_) where
    ...
    module _ {ℓ₂} {_≈_ : Rel A ℓ₂} where
      ...
      assoc refl trans antisym = Min.assoc total refl (Flip.transitive _≤_ trans) (antisymᶠ antisym)
    ...
    -- Other properties
      ...
      max[x,y]≈y⇒x≤y : _≈_ ⇒ _≤_ → Symmetric _≈_ →
                        ∀ {x y} → max x y ≈ y → x ≤ y
      max[x,y]≈y⇒x≤y refl sym = Min.min[x,y]≈y⇒y≤x total (Flip.reflexiveᵘ _ _≤_ sym refl) sym

      max[x,y]≈x⇒y≤x : _≈_ ⇒ _≤_ → Symmetric _≈_ →
                       ∀ {x y} → max x y ≈ x → y ≤ x
      max[x,y]≈x⇒y≤x refl sym = Min.min[x,y]≈x⇒x≤y total (Flip.reflexiveᵘ _ _≤_ sym refl) sym
