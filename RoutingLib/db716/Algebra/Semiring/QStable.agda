...
module RoutingLib.db716.Algebra.Semiring.QStable
  {c ℓ} (S : Semiring c ℓ) where
...
StableSemiring : ℕ → Set _
StableSemiring q = ∀ (s : Carrier) → StableElement q s
...
0-stable⇒negligibleLoops : ∀ {n} (p : Path n) (m : SquareMatrix Carrier n) (loop : HasLoop p) → StableSemiring 0
...
0-stable⇒+Idempotent : StableSemiring 0 → Idempotent _+_
0-stable⇒+Idempotent 0stab x = begin
  x + x            ≈˘⟨ +-cong (*-identityʳ x) (*-identityʳ x) ⟩
  x * 1# + x * 1#  ≈˘⟨ distribˡ x 1# 1# ⟩
  x * (1# + 1#)    ≈⟨ *-cong refl (0stab 1#) ⟩
  x * 1#           ≈⟨ *-identityʳ x ⟩
  x                ∎
  where open import Relation.Binary.Reasoning.Setoid setoid
