...
open import Data.List using (List; []; _∷_; _++_; filter; concat; foldr; gfilter) renaming (map to mapₗ)
...
module RoutingLib.Data.List.All.Properties where
  ...
  forced-map : ∀ {a b p} {A : Set a} {B : Set b} {P : B → Set p} {f : A → B} → (∀ v → P (f v)) → (xs : List A) → All P (mapₗ f xs)
  forced-map Pf []       = []
  ...
  map-preserves-predicates : ∀ {a b p} {A : Set a} {B : Set b} {P : B → Set p} {f : A → B} → (∀ x → P (f x)) → (xs : List A) → All P (mapₗ f xs)
  ...
  map-pairs : ∀ {a b ℓ₁ ℓ₂} {A : Set a} {B : Set b}  {_~₁_ : Rel A ℓ₁} {_~₂_ : Rel B ℓ₂}
              {f : A → B} → f Preserves _~₁_ ⟶ _~₂_ → AllPairs _~₁_ ⋐ AllPairs _~₂_ ∘ mapₗ f
  map-pairs f-pres []           = []
  map-pairs f-pres (x∉xs ∷ xs!) = All-map (map f-pres x∉xs) ∷ map-pairs f-pres xs!
