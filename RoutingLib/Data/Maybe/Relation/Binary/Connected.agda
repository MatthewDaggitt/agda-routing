...
module RoutingLib.Data.Maybe.Relation.Binary.Connected where
...
transʳ : Transitive R → ∀ {x y z} →
         R x y → Connected R (just y) z → Connected R (just x) z
transʳ trans Rxy (just Ryz)   = just (trans Rxy Ryz)
transʳ trans Rxy just-nothing = just-nothing

transˡ : Transitive R → ∀ {x y z} →
         Connected R x (just y) → R y z → Connected R x (just z)
transˡ trans (just Rxy)   Ryz = just (trans Rxy Ryz)
transˡ trans nothing-just Ryz = nothing-just
