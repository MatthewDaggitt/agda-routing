...
module _ {x₀ : S} (x₀∈B : x₀ ∈ B) (𝓢 : Schedule n) where
  ...
  MessagesInSameEpoch t = ∀ i j {s} → t ≤ s → η (β s i j) ≡ η t
  ...
  ...     | yes i∈ρ₁₊ₑ | yes _   | yes _ = F-mono-D test (λ j → m∈b i ep i∈ρ₁₊ₑ _) i
    ...
    test {j} j∉ρ₁₊ₑ = ≈ᵢ-reflexive (asyncIter-inactive 𝓘 𝓢 x₀ (rec (β (suc e) i j) _) (subst (λ v → j ∉ₛ π v) (trans {!!} {!!}) j∉ρ₁₊ₑ))
  ...
  messages∈Dₖᵢ⇒state∈D₁₊ₖᵢ {s} {suc e} {i} (mkₐ ηₛ≡η₁₊ₑ m s<m m≤1+e i∈αₘ)  m∈B (acc recₑ)
    with η-inRange ηₛ≡η₁₊ₑ (≤-pred (≤-trans s<m m≤1+e) , n≤1+n _)
  ... | ηₛ≡ηₑ , ηₑ≡η₁₊ₑ with i ∈? ρ (suc e) | i ∈? ρ e | i ∈? α (suc e)
  ...   | no  i∉ρ₁₊ₑ | _       | _     = D-null i∉ρ₁₊ₑ
  ...   | yes i∈ρ₁₊ₑ | no i∉ρₑ | _     = contradiction (∈ρ-subst (sym ηₑ≡η₁₊ₑ) i∈ρ₁₊ₑ) i∉ρₑ
  ...   | yes i∈ρ₁₊ₑ | yes _   | yes _ = F-mono-D (λ i∉p → {!!}) (λ j → m∈B (mkₛₑ s≤1+e ηₛ≡η₁₊ₑ) i∈ρ₁₊ₑ _) i
    where s≤1+e = ≤-trans (n≤1+n s) (≤-trans s<m m≤1+e)
  ...   | yes _       | yes _   | no  i∉α₁₊ₑ with m ≟ suc e
  ...     | yes refl  = contradiction i∈αₘ i∉α₁₊ₑ
  ...     | no  m≢1+e = async∈-resp-Dₜᵢ e ηₑ≡η₁₊ₑ (messages∈Dₖᵢ⇒state∈D₁₊ₖᵢ (mkₐ ηₛ≡ηₑ m s<m m≤e i∈αₘ) m∈B _)
  ...
  messages∈Dₖ⇒state∈D₁₊ₖ : ∀ {s e b} →
                           ...
                           MessagesInBox b AtTime s →
                           StateInBox (suc b) AtTime e
  messages∈Dₖ⇒state∈D₁₊ₖ {s} {e} active m∈b i with i ∈? ρ s 
  ... | no  i∉ρₛ = i∉ρ⇒stateᵢ∈Bₜ (i∉ρₛ ∘ ∈ρ-subst (sym (IsActivationPeriod.ηₛ≡ηₑ active)))
  ... | yes i∈ρₛ = messages∈Dₖᵢ⇒state∈D₁₊ₖᵢ (isActivation i∈ρₛ) (m∈b i) 
  ...
  messages-pp {e} {s} {b} pp m∈B i {j} {t} (mkₛₑ e≤1+t ηₑ≡η₁₊ₜ) i∈ρ₁₊ₜ accβ = begin⟨ test accβ ⟩
    ...
    ... | yes j∈ρₛ = messages∈Dₖᵢ⇒state∈D₁₊ₖᵢ (mkₐ ηₛ≡ηβ (α+ j∈ρₛ) (s<α+ j∈ρₛ) (α+≤β i i∈ρₛ j∈ρₛ e≤1+t) (i∈α+[i] j∈ρₛ)) (m∈B j)
  ...
  messages-mpp {_} {_} {_}     {b} none            s∈b = s∈b
  messages-mpp {s} {e} {suc k} {b} (next m pp mpp) s∈b = begin⟨ s∈b ⟩
    ⇒ MessagesInBox b           AtTime s ∴⟨ messages-pp pp ⟩
    ⇒ MessagesInBox (suc b)     AtTime m ∴⟨ messages-mpp mpp ⟩
  ...
  module _ {s : 𝕋} where
    ...
    messages∈Dₖ : ∀ {e k} → IsMultiPseudoperiodic (suc k) [ s , e ] → MessagesInBox k AtTime e
    messages∈Dₖ {e} {k} (next m pp mpp) = begin⟨ messages∈B s ⟩
      ⇒ MessagesInBAtTime            s ∴⟨ messages∈B-pp pp ⟩
      ⇒ MessagesInBox 0       AtTime m ∴⟨ messages-mpp mpp ⟩
    ...
    messages∈Dₖ+ : ∀ {e k} →
                   ...
                   MessagesInBox k AtTime e
    messages∈Dₖ+ {e} {k} cp = begin⟨ mpp ⟩
      ⇒ IsMultiPseudoperiodic (suc k) [ s , mid ] ∴⟨ messages∈Dₖ ⟩
    ...
    state∈Dₖ+  {e} {t} {zero} cp ep = begin⟨ messages∈B mid ⟩
      ⇒ MessagesInBAtTime mid ∴⟨ messages∈B⇒state∈D₀ active ⟩
      ⇒ StateInBox 0 AtTime e ∴⟨ state-steps-B ep (messages∈B e) ⟩
      ⇒ StateInBox 0 AtTime t ∎
      where open IsConvergentPeriod cp
    state∈Dₖ+  {e} {t} {suc k} cp ep = begin⟨ messages∈Dₖ mpp ⟩
      ⇒ MessagesInBox     k AtTime mid  ∴⟨ messages∈Dₖ⇒state∈D₁₊ₖ active ⟩
      ⇒ StateInBox    suc k AtTime e    ∴⟨ {!!} ⟩ --state-steps ep (messages∈Dₖ+ cp) ⟩
