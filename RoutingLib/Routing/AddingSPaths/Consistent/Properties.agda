...
module RoutingLib.Routing.AddingSPaths.Consistent.Properties
  ...
  abstract
    ...
    Iᶜᵢᵢ≡one[] : ∀ i → Iᶜ i i ≡ croute one [] refl
    Iᶜᵢᵢ≡one[] i with i ≟ᶠ i 
...
{-
    ...
    ... | yes _ | yes _ | yes (v , b) rewrite Iᶜᵢᵢ≡one[] l with select (v ▷ x) one
    ...
    with i ∷ p ∣ i∉p ∣ (v , subst (λ w → G i w ≡ cedge v) j≡p₀ Gᵢⱼ≡v) ≤ₚ? p
