...
open import RoutingLib.Data.Graph using (Graph; _ᵉ∈ᵍ?_; _ᵉ∈ᵍ_)
open import RoutingLib.Data.Graph.SPath  using ([]; [_]) renaming (source to sourceⁱ; _∉ₙₑₚ?_ to _∉ₙₑₚⁱ?_)
open import RoutingLib.Data.Graph.SPath.Properties using () renaming (_≤ₚ?_ to _≤ₚⁱ?_)
open import RoutingLib.Data.Graph.SGPath using ([]; [_]) renaming (source to sourceᶜ; _∉ₙₑₚ?_ to _∉ₙₑₚᶜ?_)
open import RoutingLib.Data.Graph.SGPath.Properties using () renaming (_≤ₚ?_ to _≤ₚᶜ?_)

open import RoutingLib.Data.Graph.SGPath.Equivalence
...
module RoutingLib.Routing.AlgebraicPaths.Bisimilarity
  ...
  abstract
    ...
    ⊕-≃ {iroute w p} {croute x q _} {iroute y r} {croute z s _} (routeEq w≈x p≃q) (routeEq y≈z r≃s) with select w y | select x z
    ...
    ... | sel≈ w⊕y≈w _     | sel₂ x⊕z≉x _     = contradiction (trans (trans (⊕-pres-≈ (sym w≈x) (sym y≈z)) w⊕y≈w) w≈x) x⊕z≉x
    ... | sel≈ _     _     | sel≈ _     _     with p ≤ₚⁱ? r | q ≤ₚᶜ? s
    ...   | yes _   | yes _   = routeEq w≈x p≃q
    ...   | yes p≤r | no  q≰s = contradiction (≃ₚ-≤ˢᵍ p≃q r≃s p≤r) q≰s
    ...   | no  p≰r | yes q≤s = contradiction (≃ₚ-≤ˢ  p≃q r≃s q≤s) p≰r
    ...   | no  _   | no  _   = routeEq y≈z r≃s

    ▷-≃ : ∀ i j {x y} → x ≃ y → (i , j) ▷ⁱ x ≃ (i , j) ▷ᶜ y
    ▷-≃ _ _ {inull}          {cnull}           nullEq           = nullEq
    ▷-≃ i j {iroute x []}    {croute y [] y≈1} (routeEq x≈y []) with i ≟ᶠ j | (i , j) ᵉ∈ᵍ? G
    ... | yes _ | _           = nullEq
    ... | no  _ | no  _       = nullEq
    ...
    ▷-≃ _ _ {iroute _ [ _ ]} {croute _ []    _} (routeEq x≈y ())
    ▷-≃ _ _ {iroute _ []}    {croute _ [ _ ] _} (routeEq x≈y ())
    ▷-≃ i j {iroute x [ p ]} {croute y [ q ] _} (routeEq x≈y [ p≃q ]) with j ≟ᶠ sourceⁱ p | j ≟ᶠ sourceᶜ q | i ∉ₙₑₚⁱ? p | i ∉ₙₑₚᶜ? q | (i , j) ᵉ∈ᵍ? G
    ... | no  _    | no _     | _       | _       | _           = nullEq
    ... | no  j≢p₀ | yes j≡q₀ | _       | _       | _           = contradiction (≡-trans j≡q₀ (≡-sym (≃ₙₑₚ-source p≃q))) j≢p₀
    ... | yes j≡p₀ | no  j≢q₀ | _       | _       | _           = contradiction (≡-trans j≡p₀ (≃ₙₑₚ-source p≃q)) j≢q₀
    ... | yes _    | yes _    | no  _   | no _    | _           = nullEq
    ... | yes _    | yes _    | yes i∉p | no  i∈q | _           = contradiction (≃ₙₑₚ-∉ˢᵍ p≃q i∉p) i∈q
    ... | yes _    | yes _    | no  i∈p | yes i∉q | _           = contradiction (≃ₙₑₚ-∉ˢ  p≃q i∉q) i∈p
    ...
    ic+ic⇒cc (routeEq x≈y p≃q) (routeEq x≈z p≃r) = crouteEq (trans (sym x≈y) x≈z) (≃ₚ+≃⇒≈ˢᵍ p≃q p≃r)

    ic+cc⇒ic : ∀ {x y z} → x ≃ y → z ≈ᶜ y → x ≃ z
    ic+cc⇒ic nullEq            cnullEq            = nullEq
    ic+cc⇒ic (routeEq x≈y p≃q) (crouteEq z≈y r≈q) = routeEq (trans x≈y (sym z≈y)) (≃ₚ+≈ˢᵍ⇒≃ p≃q r≈q)

    ic+ii⇒ic : ∀ {x y z} → x ≃ y → x ≈ⁱ z → z ≃ y
    ic+ii⇒ic nullEq            inullEq            = nullEq
    ic+ii⇒ic (routeEq x≈y p≃q) (irouteEq x≈z p≈r) = routeEq (trans (sym x≈z) x≈y) (≃ₚ+≈ˢ⇒≃ p≃q p≈r)
    ...
    module InconsistentReasoning where
      ...
      abstract
        ...
        _≃ⁱᶜ⟨_⟩_ : ∀ x {y z} → x ≃ y → x ≈ⁱ z → z ≃ y
        _ ≃ⁱᶜ⟨ x≃y ⟩ x≈z = ic+ii⇒ic x≃y x≈z

        _≃ᶜⁱ⟨_⟩_ : ∀ {x} y {z} → x ≃ y → z ≃ y → x ≈ⁱ z
        _ ≃ᶜⁱ⟨ x≃y ⟩ z≃y = ic+ii⇒ii x≃y z≃y
