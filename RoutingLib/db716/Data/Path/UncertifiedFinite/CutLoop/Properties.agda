...
cutLoopAux-⇿ (a , b) (a' , .b) (_ ∷ .[]) (there ()) (start ∷ pValid) refl
...
cutLoopDecreasesLength {n} ((i , j) ∷ []) (here ())
...
{-
-- have to ensure that path remains non-empty by ∷ing e on everywhere here. can I make better definitions to avoid this?
cutLoopFrom' : ∀ {n} {e : Edge n} (p : Path n) (loop : HasLoop p) (i : Fin n) → PathFrom i (e ∷ p) → PathFrom i (cutLoop (e ∷ p) (there loop))
cutLoopFrom' p loop i here = here

cutLoopAuxTo : ∀ {n} (e e' : Edge n) (p : Path n) (e'∈p : e' ∈ p) (j : Fin n) → PathTo j (e ∷ p) → proj₂ e ≡ proj₂ e' → PathTo j (e ∷ (cutLoopAux e' p e'∈p))
cutLoopAuxTo e (i , .(proj₂ e)) ((i , .(proj₂ e)) ∷ []) (here refl) .(proj₂ e) (there here) refl = here
cutLoopAuxTo e e' .(_ ∷ _) (here px) j (there (there pathTo)) e₂≡j = there pathTo
cutLoopAuxTo e e' (.(_ , j) ∷ .[]) (there ()) j (there here) e₂≡j
cutLoopAuxTo e e' (e'' ∷ p) (there e'∈p) j (there (there pathTo)) e₂≡j = cutLoopAuxTo e e' p e'∈p j (there pathTo) e₂≡j 
                                                                                                                     
cutLoopTo' : ∀ {n} {e : Edge n} (p : Path n) (loop : HasLoop p) (j : Fin n) → PathTo j (e ∷ p) → ValidPath (e ∷ p) → PathTo j (cutLoop (e ∷ p) (there loop))
cutLoopTo' {_} {fst , snd} ((.snd , .snd) ∷ .[]) trivial .snd (there here) (continue ∷ (start ∷ valid)) = here
cutLoopTo' {_} {fst , snd} ((.snd , .snd) ∷ .[]) trivial j (there (there ())) (continue ∷ (start ∷ valid))
cutLoopTo' {_} {fst , snd} ((.snd , .snd) ∷ .((snd , _) ∷ _)) trivial j (there (there pathTo)) (continue ∷ (continue ∷ valid)) = there pathTo
cutLoopTo' {_} {fst , _} ((_ , b) ∷ .[]) (here ()) j pathTo (continue ∷ (start ∷ valid))
cutLoopTo' {_} {a , b} ((b , c) ∷ (c , d) ∷ p) (here {_} {x} xb∈p) j (there (there pathTo)) (continue ∷ continue ∷ valid)
  = cutLoopAuxTo (a , b) (x , b) ((c , d) ∷ p) xb∈p j (there pathTo) refl
cutLoopTo' {_} {e} ((a , b) ∷ p) (there loop) j (there pathTo) (v0 ∷ valid) = there (cutLoopTo' p loop j pathTo valid)
-}
...
i≢j⇒cutLoopAux≢[] i j e .(e ∷ (_ , j) ∷ []) (here refl) e₂≡i (there here) i≢j ()
i≢j⇒cutLoopAux≢[] i j e .(e ∷ _ ∷ _) (here refl) e₂≡i (there (there toj)) i≢j ()
i≢j⇒cutLoopAux≢[] i j e (.(_ , j) ∷ .[]) (there ()) e₂≡i here i≢j cutloop≡[]
...
i≢j⇒cutLoop≢[] i j .((i , i) ∷ _ ∷ _) trivial here (there (there toj)) i≢j ()
i≢j⇒cutLoop≢[] i j .((i , j) ∷ []) (here ()) here here i≢j cutloop≡[]
i≢j⇒cutLoop≢[] i j (.(i , _) ∷ []) (here ()) here (there toj) i≢j cutloop≡[]
i≢j⇒cutLoop≢[] i j (.(i , _) ∷ e' ∷ p) (here {_} {a} e∈p) here (there toj) i≢j cutloop≡[] = i≢j⇒cutLoopAux≢[] i j (a , i) (e' ∷ p) e∈p refl toj i≢j cutloop≡[] 
i≢j⇒cutLoop≢[] i j (.(i , j) ∷ .[]) (there ()) here here i≢j cutloop≡[]
i≢j⇒cutLoop≢[] i j (.(i , _) ∷ p) (there loop) here (there toj) i≢j ()
...
cutLoopAuxFrom' i j e .((_ , j) ∷ []) (there ()) fromi here valid
...
cutLoopFrom' i j .((i , i) ∷ []) trivial here (there ()) (start ∷ valid)
cutLoopFrom' i j .((i , i) ∷ (i , _) ∷ _) trivial here (there toj) (continue ∷ valid) = inj₁ here
cutLoopFrom' i j ((i , k) ∷ []) (here ()) here toj valid
...
cutLoopAuxTo' i j e .((_ , j) ∷ []) (there {xs = .[]} ()) proj₂e≡i here valid
...
cutLoopTo' i j .((i , _) ∷ []) (here {[]} ()) here to valid
cutLoopTo' i j .((i , _) ∷ e ∷ p) (here {e ∷ p} {a} ai∈ep) here (there to) (x₂ ∷ valid) = cutLoopAuxTo' i j (a , i) (e ∷ p) ai∈ep refl to valid
cutLoopTo' i j .((i , j) ∷ []) (there ()) here here valid
cutLoopTo' i j .((i , k) ∷ []) (there {.[]} loop) (here {k}) (there ()) (start ∷ valid)
