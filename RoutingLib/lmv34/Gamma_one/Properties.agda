...
postulate
  ...
  ⊕ₛ-assoc : ∀ {A B C} → (A ⊕ₛ B) ⊕ₛ C ↭ A ⊕ₛ (B ⊕ₛ C)
...
⊕ₛ-identityₗ : ∀ {A} → Ø ⊕ₛ A ↭ A
⊕ₛ-identityₗ {A} = sort↭ {A}

⊕ₛ-identityᵣ : ∀ {A} → A ⊕ₛ Ø ↭ A
⊕ₛ-identityᵣ {A} = ↭-trans prf (sort↭ {A})
...
⊕ᵥ-identityₗ : ∀ {A} → Øᵥ ⊕ᵥ A ≈ᵥ A
⊕ᵥ-identityₗ i = ⊕ₛ-identityₗ

⊕ᵥ-identityᵣ : ∀ {A} → A ⊕ᵥ Øᵥ ≈ᵥ A
⊕ᵥ-identityᵣ i = ⊕ₛ-identityᵣ
...
⊕-distributive A B i = begin
  (~(A ⊕ₘ B)) i ↭⟨ ↭-refl ⟩
  (tabulate λ j → (j , (A i j) ⊕ (B i j))) †                           ↭⟨ ↭-sym (LemmaA₂-bin (λ j → A i j) (λ j → B i j)) ⟩
  ((tabulate (λ d → d , A i d)) †) ⊕ₛ ((tabulate (λ d → d , B i d)) †) ↭⟨ ↭-refl ⟩
  (~ A) i ⊕ₛ (~ B) i ∎
  where open PermutationReasoning
