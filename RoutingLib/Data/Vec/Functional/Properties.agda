...
module _ {a p} {A : Set a} (P : Pred A p) {_•_ : Op₂ A} where
  ...
  foldr⁺-forces× : _•_ Forcesᵇ P → ∀ {n} (t : Vector A (suc n)) →
...
min[t]<x : ∀ ⊤ {n} (t : Vector ℕ n) {x} → ⊤ < x ⊎ Any (_< x) t → min ⊤ t < x
min[t]<x ⊤ t (inj₁ ⊤<x) = foldr-⊎presʳ (_< _) o<m⇒n⊓o<m ⊤<x t
min[t]<x ⊤ t (inj₂ t<x) = foldr-⊎pres (_< _) n<m⊎o<m⇒n⊓o<m ⊤ t<x

min⁺[t]<x : ∀ {n} {t : Vector ℕ (suc n)} {x} → Any (_< x) t → min⁺ t < x
min⁺[t]<x = foldr⁺-⊎pres (_< _) n<m⊎o<m⇒n⊓o<m

x<min⁺[t] : ∀ {n} {t : Vector ℕ (suc n)} {x} → All (x <_) t → x < min⁺ t
x<min⁺[t] = foldr⁺-×pres (_ <_) m<n×m<o⇒m<n⊓o

min[s]<min[t] : ∀ ⊤₁ {⊤₂} {m n} {s : Vector ℕ m} {t : Vector ℕ n} → ⊤₁ < ⊤₂ ⊎ Any (_< ⊤₂) s →
...
min⁺[s]<min⁺[t] : ∀ {m n} {s : Vector ℕ (suc m)} {t : Vector ℕ (suc n)} →
...
max⁺-cong : ∀ {n} {s t : Vector ℕ (suc n)} → Pointwise _≡_ s t → max⁺ s ≡ max⁺ t
max⁺-cong s≡t = foldr⁺-cong {_~_ = _≡_} (cong₂ _⊔_) s≡t

t≤max⁺[t] : ∀ {n} (t : Vector ℕ (suc n)) → All (_≤ max⁺ t) t
...
max[t]≤x : ∀ {n} {t : Vector ℕ n} {x ⊥} → ⊥ ≤ x → All (_≤ x) t → max ⊥ t ≤ x
...
⊥≤max[t] : ∀ {n} ⊥ (t : Vector ℕ n)→ ⊥ ≤ max ⊥ t
⊥≤max[t] {n} ⊥ t = x≤max[t] {n} ⊥ t (inj₁ ≤-refl)

t≤max[t] : ∀ {n} ⊥ (t : Vector ℕ n) → All (_≤ max ⊥ t) t
...
min∞[s]≤min∞[t] : ∀ ⊤₁ {⊤₂} {m n} {s : Vector ℕ∞ m} {t : Vector ℕ∞ n} → ⊤₁ ≤∞ ⊤₂ ⊎ Any (_≤∞ ⊤₂) s → All (λ y → ⊤₁ ≤∞ y ⊎ Any (_≤∞ y) s) t → min∞ ⊤₁ s ≤∞ min∞ ⊤₂ t
min∞[s]≤min∞[t] ⊤₁ {n = zero}  v all = min∞[t]≤x ⊤₁ _ v
min∞[s]≤min∞[t] ⊤₁ {n = suc m} v all = m≤∞n×m≤∞o⇒m≤∞n⊓o
                (min∞[t]≤x ⊤₁ _ (all fzero))
                (min∞[s]≤min∞[t] ⊤₁ v (all ∘ fsuc))

x≤min∞[t] : ∀ {n x ⊤} {t : Vector ℕ∞ n} → All (x ≤∞_) t → x ≤∞ ⊤ → x ≤∞ min∞ ⊤ t
x≤min∞[t] {n} {x} {⊤} {t} all x≤⊤ = foldr-×pres (x ≤∞_) m≤∞n×m≤∞o⇒m≤∞n⊓o x≤⊤ all

min∞[t]≡x : ∀ {n x ⊤} {t : Vector ℕ∞ n} → Any (x ≡_) t → All (x ≤∞_) t → x ≤∞ ⊤ → min∞ ⊤ t ≡ x
min∞[t]≡x {n} {x} {⊤} {t} (i , x≡tᵢ) all x≤⊤ = ≤∞-antisym
          (min∞[t]≤x ⊤ t (inj₂ (i , ≤∞-reflexive (sym x≡tᵢ))))
          (x≤min∞[t] all x≤⊤)

-- Properties of sum
sum[s]≤sum[t] : ∀ {n} {s t : Vector ℕ n} → Pointwise _≤_ s t → sum s ≤ sum t
sum[s]≤sum[t] {zero} {s} {t} s≤t = z≤n
sum[s]≤sum[t] {suc n} {s} {t} s≤t = +-mono-≤ (s≤t fzero) (sum[s]≤sum[t] {n} (λ i → s≤t (fsuc i)))

sum[s]<sum[t] : ∀ {n} {s t : Vector ℕ n} → Pointwise _≤_ s t →
