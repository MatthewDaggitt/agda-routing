...
module RoutingLib.Routing.AddingSPaths.Consistent
  ...
  cnone             ▷ᶜ _                          = cnull
  _                 ▷ᶜ cnull                      = cnull
  cedge (i , j , e) ▷ᶜ (croute x [] x≈w[p]) with i ≟ᶠ j | (i , j) ᵉ∈ᵍ? G
  ... | yes _  | _              = cnull
  ... | _      | no _           = cnull
  ... | no i≢j | yes (v , Gᵢⱼ≡v) = croute (v ▷ one) [ i ∺ j ∣ i≢j ∣ (v , Gᵢⱼ≡v) ] refl
  cedge (i , j , e) ▷ᶜ (croute x [ p ] x≈w[p]) with j ≟ᶠ source p | i ∉ₙₑₚ? p | (i , j) ᵉ∈ᵍ? G
  ... | no _       | _       | _              = cnull
  ... | _          | no  _   | _              = cnull
  ... | _          | _       | no _           = cnull
  ... | yes j≡s[p] | yes i∉p | yes (v , Gᵢⱼ≡v) = croute (v ▷ x) [ i ∷ p ∣ i∉p ∣ (v , subst (λ j → G i j ≡ cedge v) j≡s[p] Gᵢⱼ≡v) ] (▷-pres-≈ v x≈w[p])
  ...
  abstract
    ...
    ▷ᶜ-pres-≈ᶜ {b = cedge (i , j , v)} {croute x [] _} {croute y [] _} (crouteEq x≈y []) with i ≟ᶠ j | (i , j) ᵉ∈ᵍ? G
    ... | yes _ | _     = ≈ᶜ-refl
    ... | no  _ | no  _ = ≈ᶜ-refl
    ... | no  _ | yes _ = ≈ᶜ-refl
    ▷ᶜ-pres-≈ᶜ {b = cedge (i , j , v)} {croute x [ p ] _} {croute y [ q ] _} (crouteEq x≈y [ p≈q ]) with j ≟ᶠ source p | j ≟ᶠ source q | i ∉ₙₑₚ? p | i ∉ₙₑₚ? q | (i , j) ᵉ∈ᵍ? G
