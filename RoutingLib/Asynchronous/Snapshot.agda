open import Data.List using (List; []; _∷_; map; concat; allFin)
...
module RoutingLib.Asynchronous.Snapshot {a ℓ n} {S : Setoid a ℓ} (p : Parallelisation S n) where
    ...
    _≈ₛ_ {β₁} {β₂} {t₁} {t₂} s₁ s₂ = ∀ t i j (βᵢⱼ≤t₁ : β₁ (suc t + t₁) i j ≤ t₁) (βᵢⱼ≤t₂ : β₂ (suc t + t₂) i j ≤ t₂) → s₁ i j (n≤m+n (suc t) t₁) βᵢⱼ≤t₁ ≈ᵢ s₂ i j (n≤m+n (suc t) t₂) βᵢⱼ≤t₂
    ...
    toListᵢⱼ-bounded : ∀ {β tₛ} → Snapshot β tₛ → (t : 𝕋) → ∀ (i j : Fin n) → List Mᵢ
    ...
    toListᵢⱼ : ∀ {β tₛ} → Dynamic β → Snapshot β tₛ → ∀ (i j : Fin n) → List Mᵢ
    toListᵢⱼ {β} {tₛ} dynamic snapshot i j = toListᵢⱼ-bounded snapshot (expiryᵢⱼ dynamic tₛ i j) i j

    toListⱼ : ∀ {β tₛ} → Dynamic β → Snapshot β tₛ → ∀ (j : Fin n) → List Mᵢ
