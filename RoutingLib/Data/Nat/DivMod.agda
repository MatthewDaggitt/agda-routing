...
%≡0⇒mod≡0 m eq = fromℕ<-cong (m%n<n m _) (s≤s z≤n) eq

n[mod]n≡0 : ∀ n → suc n mod suc n ≡ zero
n[mod]n≡0 n = fromℕ<-cong (m%n<n (suc n) n) (s≤s z≤n) (n%n≡0 n)

toℕ-mod : ∀ {n} (i : Fin (suc n)) → toℕ i mod suc n ≡ i
toℕ-mod {n} i = begin
  fromℕ< (m%n<n (toℕ i) n)  ≡⟨ fromℕ<-cong (m%n<n (toℕ i) n) (toℕ<n i) (m≤n⇒m%n≡m (≤-pred (toℕ<n i))) ⟩
  fromℕ< (toℕ<n i)          ≡⟨ fromℕ<-toℕ i (toℕ<n i) ⟩
  i                         ∎

+ˡ-mod : ∀ {a} b {n} → suc n ∣ a → (a + b) mod suc n ≡ b mod suc n
+ˡ-mod {a} b eq = fromℕ<-cong (m%n<n (a + b) _) (m%n<n b _) (%-remove-+ˡ b eq)

+ʳ-mod : ∀ a {b} {n} → suc n ∣ b → (a + b) mod suc n ≡ a mod suc n
+ʳ-mod a {b} eq = fromℕ<-cong (m%n<n (a + b) _) (m%n<n a _) (%-remove-+ʳ a eq)
