...
-- MATTHEW: much easier to prove this the following way:
--   1. Use sort↭ and ↭-trans to show: xs ↭ ys ⇒ sort xs ↭ sort ys
--   2. Prove general lemma: xs ↭ ys → Sorted xs → Sorted ys → xs ≋ ys
--      which should be pretty easy.
--   3. Combine to prove result
...
†-cong A=A' = Perm.filter⁺ FinRoute-setoid isValid? †-respects-≈ᵣ A=A'
...
... | yes valid = contradiction valid invalid
... | no  _     = refl
