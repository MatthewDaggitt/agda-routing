...
open import RoutingLib.Data.Path.Uncertified.FiniteEdge.NonEmpty as Uncertified using ([]; _∷_)

module RoutingLib.Data.Path.Certified.FiniteEdge.NonEmpty.Convert {n : ℕ} where
...
mutual
  ...
  to : Uncertified.Pathⁿᵗ n → Pathⁿᵗ n
  to []                        = []
  to ((i , j) ∷ p) = ?
  {-
  with i <? n | j <? n
  ...
  {-
  to-⇿ : ∀ {p i j} (i<n : i < n) (j<n : j < n) → (i , j) ⇿ p → (fromℕ≤ i<n , fromℕ≤ j<n) ⇿ to p
  ...
  to-∉ : ∀ {p i} (i<n : i < n) → i ∉ p → fromℕ≤ i<n ∉ to p
