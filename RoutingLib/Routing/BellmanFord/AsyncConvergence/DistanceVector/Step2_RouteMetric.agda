...
abstract
  ...
  σXᵢⱼ≉Iᵢⱼ X {i} {j} x i≢j (σXᵢⱼ≤x , σXᵢⱼ≉x) σXᵢⱼ≈Iᵢⱼ =
    σXᵢⱼ≉x (≤₊-antisym σXᵢⱼ≤x (begin
      x         ≤⟨ ⊕-identityˡ _ ⟩
      ∞         ≈⟨ ≈-sym (≈-reflexive (Iᵢⱼ≡∞ (i≢j ∘ sym))) ⟩
      I i j     ≈⟨ ≈-sym σXᵢⱼ≈Iᵢⱼ ⟩
      σ X i j   ∎))
    where open PO-Reasoning ≤₊-poset

  d-strContr-lemma : ∀ {X Y r s} →
                      (∀ u v → X u v ≉ Y u v → d (X u v) (Y u v) ≤ h (X r s) ⊔ h (Y r s)) →
                      ∀ {i j} → σ X i j <₊ σ Y i j →
                      h (σ X i j) ⊔ h (σ Y i j) < h (X r s) ⊔ h (Y r s)
  d-strContr-lemma {X} {Y} {r} {s} d≤hXᵣₛ⊔hYᵣₛ {i} {j} σXᵢⱼ<σYᵢⱼ@(σXᵢⱼ≤σYᵢⱼ , σXᵢⱼ≉σYᵢⱼ)
    with i ≟𝔽 j
  ...
  ...   | inj₁ σXᵢⱼ≤σYᵢⱼ = d-strContr-lemma d≤dXᵣₛYᵣₛ (σXᵢⱼ≤σYᵢⱼ , σXᵢⱼ≉σYᵢⱼ)
  ...   | inj₂ σXᵢⱼ≤σYᵢⱼ = begin
    h (σ X i j) ⊔ h (σ Y i j) ≡⟨ ⊔-comm (h (σ X i j)) (h (σ Y i j)) ⟩
    h (σ Y i j) ⊔ h (σ X i j) <⟨ d-strContr-lemma d≤dYᵣₛXᵣₛ (σXᵢⱼ≤σYᵢⱼ , (σXᵢⱼ≉σYᵢⱼ ∘ ≈-sym)) ⟩
