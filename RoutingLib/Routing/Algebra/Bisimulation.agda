...
module RoutingLib.Routing.Algebra.Bisimulation
  ...
  (Aᵃ : SquareMatrix (Step 𝓐) n)
  (Aᵇ : SquareMatrix (Step 𝓑) n)
  where
  ...
  record Bisimilar₁ : Set (lsuc (a₁ ⊔ a₂ ⊔ b₁ ⊔ b₂ ⊔ ℓ₁ ⊔ ℓ₂))where
    ...
    field
      ...
      to-▷      : ∀ i j x → to (Aᵃ i j  ▷ᵃ x) ≈ᵇ Aᵇ i j ▷ᵇ to x
  ...
  module _ (𝓐↭𝓑 : Bisimilar₁) where
    ...
    to-σ {i} X j = begin
      ...
      ≈⟨ foldr⁺ (S 𝓑) (⊕-cong 𝓑) (≈-refl 𝓑) (ListEq.tabulate⁺ (λ k → to-▷ i k (X k j)) ) ⟩
