...
module RoutingLib.Data.Path.Certified.FiniteEdge.NonEmpty.Properties where
...
⇿-sub : ∀ {n i j k} {p : Pathⁿᵗ n} → (i , j) ⇿ p → k ≢ j → (k , j) ⇿ p
...
p≉i∷p : ∀ {n e} {p : Pathⁿᵗ n} {e⇿p e∉p} → ¬ (p ≈ₚ e ∷ p ∣ e⇿p ∣ e∉p)
p≉i∷p {p = []}            ()
