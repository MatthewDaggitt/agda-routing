...
module RoutingLib.Routing.BellmanFord.PathsConvergence2.SufficientConditions  where
  ...
  record PathSufficientConditions
    ...
    field
      ...
      path-cong         : ∀ {r s} → r ≈ s → (r≉0 : r ≉ 0#) (s≉0 : s ≉ 0#) →
                          path r≉0 ≈ₚ path s≉0
      path-looping      : ∀ {i} j {r} (r≉0 : r ≉ 0#) → i ∈ path r≉0 →
                          A i j ▷ r ≈ 0#
      path-extension₁   : ∀ {i j r} (r≉0 : r ≉ 0#) (Aᵢⱼ▷r≉0 : A i j ▷ r ≉ 0#) →
                         path r≉0 ≈ₚ [] → ∃ λ i≢j → path Aᵢⱼ▷r≉0 ≈ₚ [ i ∺ j ∣ i≢j ]
      path-extension₂   : ∀ {i j r} (r≉0 : r ≉ 0#) (Aᵢⱼ▷r≉0 : A i j ▷ r ≉ 0#) →
                         ∀ {p} → path r≉0 ≈ₚ [ p ] →
    ...
    -}
      ...
      -- Maximum nesting
      postulate d : ℕ
      postulate confederationSizes : (currentNode : Fin n) → (depth : Fin d) → ℕ
      
      npath : ∀ {r : Route} → r ≉ 0 → (depth : Fin d) → SimplePath (confederationSizes (source (path r≉0)) depth)
      npath-cong : bleh bleh
