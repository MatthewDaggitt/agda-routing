...
module RoutingLib.Routing.AddingSPaths.Inconsistent.Properties
  ...
  abstract
    ...
    ⊕ⁱ-almost-strictly-absorbs-▷ⁱ abs (iedge (i , j , v)) {iroute x []} _ with i ≟ᶠ j
    ... | yes i≡j = ≈ⁱ-refl , λ()
    ... | no  i≢j with select (v ▷ x) x
    ...   | sel₁ _ vx⊕x≉x = contradiction (abs v x) vx⊕x≉x
    ...   | sel₂ _ _      = ≈ⁱ-refl , λ{(irouteEq x≈vx ())}
    ...   | sel≈ _ _      = ≈ⁱ-refl , (λ{(irouteEq x≈vx ())})
    ⊕ⁱ-almost-strictly-absorbs-▷ⁱ abs (iedge (i , j , v)) {iroute x [ p ]} _  with j ≟ᶠ source p | i ∉ₙₑₚ? p
    ... | no  _    | _       = ≈ⁱ-refl , λ()
    ... | yes _    | no  _   = ≈ⁱ-refl , λ()
